...
| `/join/[shareCode]` | 초대 진입 | 공유 코드 검증, 로그인/회원가입/OAuth 후 검사 유도 | 공개 |
...
#### 동행자
- 공유 링크 유입 시 비게스트 계정 로그인/회원가입/OAuth 요구
- 로그인 계정 기준으로 참여 방과 TPTI 결과를 연결
...
| POST | `/api/rooms/:shareCode/join` | 비게스트 로그인 | 방 참여 |
